...
// if (is_string($spans_name2[10])) { b_match_status b_match_info-elem
  for ($i = 0; $i <= 2; $i++) {
    ...
    $link = array();foreach($html->find('a') as $element) {array_push($link, $element->href);}
...
$minuto = $final[$total];if ($total){$minuto2 = $final[$total-1];}else{$minuto2 = 0;}
...
?>
<h5><?php if ( strlen($minuto) <= 5){if (strlen($final[$total]) > 0) {  echo $final[$total].' - ';}else{if ($suma >= 0){echo date('H:i', $date[$total-$suma+$t]).' - ';}}} echo$spans_name2[$loc].' '.$resultado[$loc] .'vs'.' '.$resultado[$vis].' '. $spans_name2[$vis];  ?></h5>
<?
echo $link[$a];
$a=$a+2;
switch ($i) {
