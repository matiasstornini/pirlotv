...
require_once('../../simple_html_dom.php');$getdata = htmlspecialchars($_GET["url"]);
    ...
    $jugadores = array();
    foreach($html->find('div[class="fll b_lineup_players b_trim_inner -left"]') as $element) {
      array_push($jugadores, $element->plaintext);
    }
    ...
    if (is_null($est[0])) { echo 'No hay datos cargados';}
	$l = [$lo = 0, $vi = 1];
	if ($lineup) {

	    echo '<h4 align="center"><b>'.$h2[0].' '.$h2[2].' '.$h2[1].'</b></h4>';for ($i=0; $i < 9; $i++) {echo '<div align="center">'.$jugadores[$l[0]]. ' ' .$jugadores[$l[1]].'<br></div>';$l[0]=$l[0]+2;$l[1]=$l[1]+2;}
	}
?>
</div>
